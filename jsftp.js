 ...
 */
Ftp.prototype.get = function(remotePath, localPath, win, name, callback = NOOP) {
  let finalCallback;
  let timmer
  ...
  if (typeofLocalPath === "function") {
    ...
    finalCallback = (err, socket) => {
      ...
      console.log('FTP_download_progress init', { 
        name: name, 
        progress: writeStream.bytesWritten
      })

      win.webContents.send('ftpResp', 'download_progress', {           
        name: name, 
        progress: writeStream.bytesWritten
      })

      timmer = setInterval( () => { 
        console.log('FTP_download_progress', { 
          name: name, 
          progress: writeStream.bytesWritten
        })
        win.webContents.send('ftpResp', 'download_progress', { 
      ...
      writeStream.on("finish", () =>{
        console.log('FTP_download_progress finish', { 
          name: name, 
          progress: writeStream.bytesWritten
        })
        win.webContents.send('ftpResp', 'download_progress', {           
...
Ftp.prototype.ls = function(filePath, callback) {
  function entriesToList(err, entries) {
    if (err) {
      console.error("ftp: .ls", err)
      return callback(err);
    }


    console.error("ftp: .ls", filePath)
