...
define([
    ...
    internalProto.onData = function (data) {
        ...
        packetContentList = packet.parsePackets(data, this.isEvent);

        if (packetContentList === false) {
            this.logMsg('packetContentList=' + packetContentList);
            return;
        }

        packetContentList.forEach(function (packetContent) {
            //console.log('packetContent=', packetContent);
    ...
    internalProto.openSocket = function () {
        console.log("loop.openSocket.socket.isClosed=" + this.socket.isClosed);
