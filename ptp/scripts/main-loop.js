...
define(['./packet', './loop-factory'], function (packet, loopFactory) {
    ...
    openSession = function () {
        //console.log('##### mainLoop.openSession #####');
        cmdResponseCallbacks[packet.transactionId] = function (content) {
            //console.log('##### mainLoop.openSession callback #####', content);
            if (content.responseCode === loop.responseCodes.ok) {
                onSessionOpened();
            } else if (content.responseCode === 0x201e) {
                // session already opened
                onSessionOpened();
            } else if (content.responseCode === 0x201d) {
                // ???
                onSessionOpened();
            } else {
                console.log('##### mainLoop.openSession callback responseCode=' + content.responseCode);
            }
